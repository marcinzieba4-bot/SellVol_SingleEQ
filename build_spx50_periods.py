"""
build_spx50_periods.py
----------------------
For each ~4-week option period (using SPY's S3 period grid as reference),
determine the 50 largest S&P 500 stocks by market cap.
...
For the TOP-50 ranking, a ±20% error in mktcap at any point is inconsequential.

Outputs:
  spx50_periods.json   – per-period top-50 list with cap weights
  required_tickers.txt – unique ticker set across all periods (options data needed)
"""
...
OUT_JSON  = "spx50_periods.json"
OUT_TICKS = "required_tickers.txt"
...
print(f"Universe: {len(UNIVERSE)} candidate tickers")
...
# ── Build top-50 per period ────────────────────────────────────────────────────
print("\nBuilding top-50 ranking for each period...")
spx50_periods = {}
...
for period_start in period_dates:
    ...
    ranked    = sorted(mktcaps.items(), key=lambda x: x[1], reverse=True)[:50]
...
print(f"  Built top-50 for {len(spx50_periods)} periods")
...
with open(OUT_TICKS, "w") as f:
    f.write(f"# Unique tickers in SPX Top-50 across all periods\n")
...
print("\n" + "=" * 65)
print("  COMPOSITION SNAPSHOTS (top-10 shown)")
print("=" * 65)
