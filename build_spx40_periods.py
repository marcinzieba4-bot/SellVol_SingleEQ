...
For the TOP-40 ranking, a ±20% error in mktcap at any point is inconsequential.

Outputs:
  spx40_periods.json        – per-period top-40 list with cap weights
  spx40_required_tickers.txt – unique ticker set across all periods (options data needed)
"""
...
OUT_JSON  = "spx40_periods.json"
OUT_TICKS = "spx40_required_tickers.txt"

# ── Candidate universe: all stocks that could be in top 40 over 2020-2026 ─────
# ~126 tickers covering current S&P 500 top-100 + historical top-40 entrants
_UNIVERSE_RAW = [
...
# ── Build top-40 per period ────────────────────────────────────────────────────
print("\nBuilding top-40 ranking for each period...")
spx40_periods = {}
...
for period_start in period_dates:
    ...
    ranked    = sorted(mktcaps.items(), key=lambda x: x[1], reverse=True)[:40]
    total_cap = sum(v for _, v in ranked)

    spx40_periods[period_start] = [
        {
            "rank":     rank,
            "ticker":   tk,
...
print(f"  Built top-40 for {len(spx40_periods)} periods")

# ── Save JSON ──────────────────────────────────────────────────────────────────
with open(OUT_JSON, "w") as f:
    json.dump(spx40_periods, f, indent=2)
print(f"\n  Saved -> {OUT_JSON}")

# ── Required tickers ──────────────────────────────────────────────────────────
all_tickers = sorted({s["ticker"] for stocks in spx40_periods.values() for s in stocks})
