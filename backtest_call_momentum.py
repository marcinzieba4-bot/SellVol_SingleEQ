"""
backtest_call_momentum.py
--------------------------
Calls-only momentum strategy:
  Positive 4W momentum → BUY  CALL  (pay premium, receive call payoff)
  Negative 4W momentum → SELL CALL  (collect premium, pay call payoff)
8 equities, equal weighted, no SPY leg.
"""
...
W         = 1 / len(EQUITIES)
OUT_TABLE = "call_momentum_monthly.csv"
OUT_CHART = "call_momentum_chart.png"

# ── Load ──────────────────────────────────────────────────────────────────────
print("=" * 62)
print("  CALL-MOMENTUM BACKTEST")
print("  8 stocks · buy call ↑ / sell call ↓ · equal weighted")
print("=" * 62)

results = {}
for tk in EQUITIES:
    results[tk] = run_strategy(tk, mode="call_momentum")
...
for tk in EQUITIES:
    ...
    sc   = [r["pnl_pct"] for r in res if r["trade"] == "SELL CALL"]
    tk_stats[tk] = dict(
        n_buy  = len(bc), n_sell = len(sc),
        avg_buy  = np.mean(bc) if bc else 0,
        avg_sell = np.mean(sc) if sc else 0,
        total    = sum(pnls),
...
print(f"\n  PER-TICKER BREAKDOWN")
print(f"  {'Ticker':<7} {'BuyCall':>8} {'AvgBuy':>8} {'SellCall':>9} {'AvgSell':>8}  "
      f"{'Total%':>7}  {'WinRate':>7}")
print(f"  {'─'*65}")
for tk, s in tk_stats.items():
    print(f"  {tk:<7} {s['n_buy']:>8} {s['avg_buy']:>+7.2f}%"
          f" {s['n_sell']:>9} {s['avg_sell']:>+7.2f}%"
...
ax1.annotate(f"{final:+.1f}%", xy=(mo_idx[-1], final),
             ...
             color="#ffd740", fontsize=11, fontweight="bold",
...
fig.text(0.987, 0.97, box, ha="right", va="top", fontsize=8.5,
         ...
         bbox=dict(boxstyle="round,pad=0.5", facecolor="#1a1a2e",
                   edgecolor="#ffd740", alpha=0.92))

sub = (f"AAPL · AMD · AMZN · JPM · LLY · NVDA · TSLA · XOM  |  "
       f"1/8 each  |  Buy Call ↑ / Sell Call ↓  |  Sep 2020 – Feb 2026  |  NO SPY leg")
